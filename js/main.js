...
var camera, controls, scene, renderer, cameraPlaceholder, cameraPlaceholderHelper, gameCameraTarget, planes;
...
var cube, terrain, terrain2, terrain3, ground;
...
//    cameraPlaceholderHelper.visible = true;
    ...
    light.shadowCameraNear = -60; // -20
    light.shadowCameraFar = 60; // 60
    light.shadowCameraLeft = -60; // -24
    light.shadowCameraRight = 60; // 24
    light.shadowCameraTop = 60; // 24
    light.shadowCameraBottom = -60; // -24
...
function createScene( ) {
    ...
    ground = createRandomPlane(terrainSize, terrainSize, planeMaterial, .1, 3);
...
function lockPlayerZ() {
    var r = new THREE.Raycaster(new THREE.Vector3(player.position.x, player.position.y, player.position.z + 1000), new THREE.Vector3( player.position.x, player.position.y, player.position.z - 1000), -2000, 2000);
    var c = r.intersectObjects(planes);
    if (c.length > 0) {
        var zMax = null;
        ...
        var diff = zMax - player.position.z;
...
function onMouseScroll(event, delta, deltaX, deltaY) {
    if (deltaY > 0) {
        ...
        camera.position.mul.multiplyScalar(1.1);
    } else if (deltaY < 0) {
       //scroll down
